...
import { PhotoClusteringService } from '../clustering/PhotoClusteringService.js';
...
export class EventService {
  constructor() {
    ...
    this.clusteringService = new PhotoClusteringService();
  }
  ...
  /**
   * Smart auto-detect using DBSCAN clustering
   *
   * Uses density-based clustering to find natural event boundaries.
   * Better than simple time gaps because it considers photo density patterns.
   *
   * @param {Object} options - Clustering options
   * @param {number} options.epsilon - Max time gap in minutes (auto if null)
   * @param {number} options.minPoints - Min photos per event (default: 3)
   * @param {string} options.method - 'dbscan' or 'gaps' (default: 'dbscan')
   * @returns {Promise<Array>} Suggested event clusters with smart naming
   */
  async autoDetectEventsSmart(options = {}) {
    const { epsilon = null, minPoints = 3, method = 'dbscan' } = options;

    // Get all photos with capture time
    ...
    // Auto-suggest epsilon if not provided
    const suggestedEpsilon = epsilon || this.clusteringService.suggestEpsilon(photos);

    console.log(`[EventService] Smart clustering with epsilon=${suggestedEpsilon}min, minPoints=${minPoints}, method=${method}`);

    // Use clustering service
    const clusters = this.clusteringService.clusterPhotosByTime(photos, {
      epsilon: suggestedEpsilon,
      minPoints,
    });

    console.log(`[EventService] Found ${clusters.length} event(s) using ${method} clustering`);
    return clusters;
  }

  /**
   * Auto-detect event boundaries from photo timeline (simple gap-based)
   * Identifies time gaps between photos and suggests event splits
   *
   * NOTE: Consider using autoDetectEventsSmart() for better results with DBSCAN
   *
