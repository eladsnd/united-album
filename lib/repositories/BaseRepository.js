...
export class BaseRepository {
  ...
  async findUnique(where) {
    try {
      const client = this._getClient();
      console.log(`[${this.constructor.name}] Attempting findUnique:`, { where });
      const record = await client.findUnique({ where });
      console.log(`[${this.constructor.name}] FindUnique result:`, record ? 'Found' : 'Not found');
      return record ? this.deserialize(record) : null;
    } catch (error) {
      console.error(`[${this.constructor.name}] FindUnique REAL error:`, error);
      console.error(`[${this.constructor.name}] Error name:`, error.name);
      console.error(`[${this.constructor.name}] Error message:`, error.message);
      console.error(`[${this.constructor.name}] Error code:`, error.code);
      console.error(`[${this.constructor.name}] Full error:`, JSON.stringify(error, null, 2));
      throw new InternalServerError(
  ...
  async upsert(where, create, update) {
    try {
      ...
      const client = this._getClient();
      console.log(`[${this.constructor.name}] Attempting upsert:`, { where, create, update });
      const record = await client.upsert({
        where,
        create: serializedCreate,
        update: serializedUpdate,
      });
      console.log(`[${this.constructor.name}] Upsert succeeded:`, record);
      return this.deserialize(record);
    } catch (error) {
      console.error(`[${this.constructor.name}] Upsert failed with REAL error:`, error);
      console.error(`[${this.constructor.name}] Error name:`, error.name);
      console.error(`[${this.constructor.name}] Error message:`, error.message);
      console.error(`[${this.constructor.name}] Error code:`, error.code);
      console.error(`[${this.constructor.name}] Error meta:`, error.meta);
      console.error(`[${this.constructor.name}] Full error:`, JSON.stringify(error, null, 2));
      throw new InternalServerError(
