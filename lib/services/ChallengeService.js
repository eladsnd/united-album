...
export class ChallengeService {
  ...
  /**
   * Get all pose challenges (event-scoped)
   *
   * Returns global challenges + event-specific challenges for the given event
   *
   * @param {string} eventId - Event ID (optional - if not provided, returns only global challenges)
   * @returns {Promise<Array>} Array of pose challenges
   */
  async getAllPoses(eventId = null) {
    try {
      const challenges = eventId
        ? await this.challengeRepo.findForEvent(eventId)
        : await this.challengeRepo.findAll(); // Fallback: all challenges
