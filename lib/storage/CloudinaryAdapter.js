...
export class CloudinaryAdapter extends StorageAdapter {
  ...
  async upload(fileBuffer, fileName, options = {}) {
    const { folder = 'united-album', mimeType = 'image/jpeg', publicId = null } = options;

    try {
      // Convert buffer to base64
      const base64File = `data:${mimeType};base64,${fileBuffer.toString('base64')}`;

      // Use provided publicId or generate from fileName (without extension)
      const finalPublicId = publicId || fileName.replace(/\.[^/.]+$/, '');

      // Generate timestamp and signature for authenticated upload
      const timestamp = Math.round(Date.now() / 1000);
      const signature = this._generateSignature(timestamp, folder, finalPublicId);
