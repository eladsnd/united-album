...
export class PhotoRepository extends BaseRepository {
  ...
  serialize(data) {
    const serialized = {
      ...
      // Ensure capturedAt is Date object (or null)
  ...
  /**
   * Get photos filtered by eventId
   *
   * @param {string} eventId - Event ID to filter by
   * @returns {Promise<Array>} Photos assigned to this event
   */
   ...
   *
   * @returns {Promise<Array>} Photos without eventId
   */
  ...
  /**
   * Get photos captured within a date range
   * Orders by capturedAt (actual photo capture time from EXIF)
   *
   * @param {Date|string} start - Range start date
   * @param {Date|string} end - Range end date
   * @returns {Promise<Array>} Photos captured in range
   */
  ...
  /**
   * Bulk update eventId for multiple photos
   * Used for assigning photos to events
   *
   * @param {Array<number>} photoIds - Array of photo database IDs
   * @param {string|null} eventId - Event ID to assign (or null to unassign)
   * @returns {Promise<{count: number}>} Number of updated photos
   */
  async updateEventId(photoIds, eventId) {
    try {
      ...
      console.log(`[PhotoRepository] Updated ${result.count} photos with eventId: ${eventId}`);
      return result;
    } catch (error) {
      console.error(`[${this.constructor.name}] UpdateEventId failed:`, error);
      throw error;
    }
  }

  /**
   * Get all photos with capturedAt set, ordered chronologically
   * Useful for timeline visualization
   *
   * @returns {Promise<Array>} Photos with capturedAt, ordered by capture time
   */
  async findAllByCaptureTime() {
    return await this.findMany({
      where: {
        capturedAt: {
          not: null
