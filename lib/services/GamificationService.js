...
export class GamificationService {
  ...
  async awardPointsForPhoto(photo) {
    try {
      ...
      const pointsToAward = challenge.points || 10;

      // 5. Award points to user
      const updatedScore = await this.scoreRepo.addPoints(userId, challengeId, pointsToAward);

      // 6. Log achievement
      console.log(`[GamificationService] 🎉 User ${userId} earned ${pointsToAward} points for completing "${challenge.title}" (total: ${updatedScore.totalPoints})`);
  ...
  async isGamifyModeEnabled() {
    return await this.settingsRepo.isGamifyModeEnabled();
  }
   ...
   *
   * @param {Object} updates - Settings to update (e.g., { gamifyMode: true })
   * @returns {Promise<Object>} Updated settings
   */
  async updateSettings(updates) {
    return await this.settingsRepo.updateSettings(updates);
  }

  /**
   * Toggle gamify mode on/off
   *
   * @returns {Promise<Object>} Updated settings with new gamifyMode value
   */
  async toggleGamifyMode() {
    return await this.settingsRepo.toggleGamifyMode();
  }
