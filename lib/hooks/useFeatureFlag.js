...
export function useFeatureFlag(feature, options = {}) {
  ...
  useEffect(() => {
    ...
    async function fetchFlag() {
      try {
        // Fetch from event-scoped settings endpoint
        const res = await fetch(`/api/events/${currentEventId}/settings`);
        ...
        if (mounted) {
          // Extract the specific feature flag from settings
          const settings = data?.data ?? {};
          const isEnabled = settings?.[feature] ?? defaultValue;
  ...
  useEffect(() => {
    ...
    async function fetchFlags() {
      try {
        ...
        if (mounted) {
          // Extract flags from settings data
          ...
          setError(null);
          console.log(`[useFeatureFlags] Loaded flags for event ${currentEventId}:`, {
            gamification: fetchedFlags?.gamification,
            challenges: fetchedFlags?.challenges,
            bulkUpload: fetchedFlags?.bulkUpload,
          });
        }
      } catch (err) {
