...
export class ChallengeService {
  ...
  async createPose({ title, instruction, image, folderId = null }) {
    ...
    // Save image file to Google Drive (returns Drive file ID)
    const imageDriveId = await this._saveImageFile(image, id, folderId);
  ...
  async updatePose(id, { title, instruction, image, folderId }) {
    ...
    if (image && image instanceof File) {
      const newImageDriveId = await this._saveImageFile(image, id, folderId);
      updateData.image = newImageDriveId;
    }
  ...
  async deletePose(id) {
    ...
    try {
      ...
      // Try to delete image from Google Drive
      // Extract Drive ID from image path (e.g., "/api/image/1ABC123" or "1ABC123")
      const driveId = deletedPose.image.replace('/api/image/', '').replace('/challenges/', '');

      if (driveId && driveId.length > 10) { // Basic validation of Drive ID
        try {
          const { deleteFromDrive } = await import('../googleDrive.js');
          await deleteFromDrive(driveId);
          imageDeleted = true;
          note = 'Pose and image deleted successfully from Google Drive.';
          console.log(`[ChallengeService] Deleted image from Drive: ${driveId}`);
        } catch (driveError) {
          // Don't fail the whole operation if Drive deletion fails
          console.warn(`[ChallengeService] Failed to delete image from Drive (${driveId}):`, driveError.message);
          note = 'Pose deleted from database. Image may still exist in Google Drive (delete failed or file not found).';
        }
      } else {
        note = 'Pose deleted from database. Image path invalid or not in Google Drive.';
        console.log(`[ChallengeService] No valid Drive ID found in image path: ${deletedPose.image}`);
      }
  ...
  async _saveImageFile(file, poseId, folderId = null) {
    try {
      ...
      // Use provided folderId or default from environment
      ...
      // Find or create "challenges" subfolder in Google Drive
      console.log(`[ChallengeService] Finding/creating challenges folder in Drive...`);
      ...
      console.log(`[ChallengeService] Uploading pose image "${filename}" to Drive...`);
      const driveFile = await uploadToDrive(buffer, filename, challengesFolderId);

      console.log(`[ChallengeService] Uploaded pose image: ${driveFile.id}`);
      ...
      console.error('[ChallengeService] Error uploading image file to Drive:', error);
      throw new InternalServerError(
        'Failed to upload image file to Google Drive',
        'FILE_UPLOAD_FAILED'
      );
    }
  }
