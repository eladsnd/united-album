...
export function withFeature(handler, feature, options = {}) {
  ...
  return async function featureGatedHandler(request, context) {
    ...
    // Feature disabled - return empty array for consistency
    if (!enabled) {
      return NextResponse.json({
        success: true,
        data: [],  // Always return array for consistency with enabled state
        message: `Feature "${feature}" is not enabled`,
        featureEnabled: false,  // Add flag at root level instead of nesting
      });
    }
