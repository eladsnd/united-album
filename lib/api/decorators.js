...
import { isAdminAuthenticated } from '../adminAuth'; // Legacy - keep for backward compat
import { getUserFromRequest } from '../auth/sessionManager.js';
import { isSuperAdmin, isEventAdmin } from '../auth/permissions.js';
 ...
 * Uses JWT authentication (multi-tenancy mode) with fallback to legacy password auth.
 * Returns 401 if authentication fails.
 *
 * Accepts SUPER_ADMIN or EVENT_ADMIN roles.
 *
 * @param {Function} handler - Async API route handler
...
export function withAdminAuth(handler) {
  return async (request, context) => {
    // Try JWT authentication first (new multi-tenancy system)
    const user = getUserFromRequest(request);

    if (user && (isSuperAdmin(user) || isEventAdmin(user))) {
      // Attach user to request for handler to use
      request.user = user;
      return handler(request, context);
    }

    // Fallback to legacy password-based auth (backward compatibility)
