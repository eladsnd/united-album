...
export class PhotoService {
   ...
   *
   * Workflow:
   * 1. Validate photoId and eventId
   * 2. Find photo in database (event-scoped)
   * 3. Verify photo belongs to event
   * 4. Check permissions (admin OR photo owner)
   * 5. Delete from storage
   * 6. Delete from database
   * 7. Clean up orphaned faces and their thumbnails
   *
   ...
   * @param {string} eventId - Event ID for multi-tenancy (required)
   * @returns {Promise<{success: boolean, deletedPhoto: Object, orphanedFaces: string[], message: string}>}
   * @throws {ValidationError} When photoId or eventId is invalid
   * @throws {NotFoundError} When photo doesn't exist
   * @throws {UnauthorizedError} When user lacks permission or photo belongs to different event
   * @throws {InternalServerError} When deletion fails
   */
  async deletePhoto(photoId, uploaderId, isAdmin, eventId) {
    // Step 1: Validate photoId and eventId
    ...
    console.log(`[PhotoService] Delete request for photo ${photoId} in event ${eventId} by ${isAdmin ? 'ADMIN' : `uploader ${uploaderId}`}`);

    // Step 2: Find the photo (event-scoped)
    const eventPhotos = await this.photoRepo.findMany({
      where: { eventId }, // CRITICAL: Filter by eventId to prevent cross-event deletion
    });
